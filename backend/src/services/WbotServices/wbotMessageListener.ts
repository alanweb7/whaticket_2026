...
if (!isNil(flow) && isQuestion && !msg.key.fromMe) {
  ...
  if (body) {
    ...
    if (!nodeSelected) {
      logger.warn("⚠️ Nenhum nodeSelected encontrado para a pergunta atual.");
      return;
    }

    // 🔹 Força o cast do typebotIntegration
    const { message, answerKey } =
      (nodeSelected.data?.typebotIntegration as any) || {};

    if (!answerKey) {
      logger.warn("⚠️ answerKey ausente no nodeSelected.");
      return;
    }

    // 🔹 Garante que ticket.dataWebhook existe
    const oldVars =
      (ticket.dataWebhook as any)?.variables &&
      typeof (ticket.dataWebhook as any).variables === "object"
        ? (ticket.dataWebhook as any).variables
        : {};
    ...
    // 🔹 Atualiza o objeto local em memória também
    ticket.dataWebhook = { variables: newVars };

    console.log(`✅ Capturada resposta [${answerKey}] = ${body.trim()}`);

    const nodeIndex = nodes.findIndex((node) => node.id === nodeSelected.id);
    const nextNode = nodes[nodeIndex + 1];
    const lastFlowId = nextNode?.id;

    if (!lastFlowId) {
      logger.warn("⚠️ Nenhum próximo nó encontrado após question.");
      return;
    }

    await ticket.update({ lastFlowId });
    await ticket.save();
    ...
    // 🔹 Passa as variáveis atualizadas diretamente
    await ActionsWebhookService(
      whatsapp.id,
      parseInt(ticket.flowStopped),
      ticket.companyId,
      ...
      connections,
      lastFlowId,
      { variables: newVars }, // ✅ Agora o ActionsWebhook já recebe o valor do question
      "",
      "",
      "",
      ticket.id,
